...
function generateRandomDataChunk() {
    ...
    const chunkSizeInBytes = 500 * megabyte; // chunk size

    const randomData = crypto.randomBytes(chunkSizeInBytes);
    return randomData;
}

// Example of sending a chunk of random data
    ...
    client.on("connect", async () => {
        setInterval(async () => {
            ...
            function createHeadersFrame(chunk, streamId, dynHeaders) {
              ...
              const flagsStr = getFlags('HEADERS', flagsByte); // T?o chu?i c? t? flagsByte
    dynHeaders.flags = flagsStr; // Thêm chu?i c? vào dynHeaders
